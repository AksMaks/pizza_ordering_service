...
router.post('/', [], upload.Additive.single('Image'), (req, res) => {
  try{
    (async () => {
      res.status(200).json(await Additive.Insert({...req.body, Url: req.file? req.file.path: req.body.OldImage}))
    })()
  }catch(e){
    ErrorHandler(res, e)
  }
...
router.patch('/', [], upload.Additive.single('Image'), (req, res) => {
  try{
    (async () => {
      res.status(200).json(await Additive.Update({...req.body, Url: req.file? req.file.path: req.body.OldImage}))
    })()
  }catch(e){
    ErrorHandler(res, e)
  }
